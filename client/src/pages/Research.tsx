...
// --- 1. تعریف Interfaceها برای هماهنگی با مدل‌های بک‌اند ---
interface ResearchPost {
  _id: string;
  title: string;
  authors: string[];
  journal: string;
  publishedAt: string; // تاریخ به صورت رشته از API دریافت می‌شود
  category: string;
  summary: string;
  citations: number;
  downloadUrl?: string;
...
// <<-- 2. ایجاد یک Map برای تبدیل نام آیکون به کامپوننت واقعی -->>
...
const Research = () => {
  // --- 2. Stateها برای مدیریت داده‌های داینامیک ---
  const [posts, setPosts] = useState<ResearchPost[]>([]);
  const [caseStudies, setCaseStudies] = useState<CaseStudy[]>([]);
  const [stats, setStats] = useState<ResearchStat[]>([]); // <<-- State جدید برای آمارها
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState(""); // <<-- State برای جستجو

  // --- 3. دریافت داده از API با useEffect ---
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        // فراخوانی همزمان هر دو اندپوینت برای سرعت بیشتر
        const postsPromise = api.get('/posts');
        const caseStudiesPromise = api.get('/case-studies');
        const configPromise = api.get('/config');

        const [postsResponse, caseStudiesResponse, configResponse] = await Promise.all([
          postsPromise,
          caseStudiesPromise,
          configPromise,
        ]);
        ...
        setStats(configResponse.data.researchStats); // <<-- ذخیره آمارها در state
        setError(null);
      } catch (err) {
        setError('Failed to fetch research data. Please ensure the server is running.');
        console.error(err);
      } finally {
        setLoading(false);
      }
  ...
  // بخش‌های JSX بدون تغییر باقی مانده‌اند و فقط بخش‌های رندر لیست‌ها تغییر کرده
      ...
      <section className="py-20 relative" id="case-studies">
        <div className="container mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 glow-text-secondary">Real-World Impact</h2>
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">Case studies showcasing how our AI research translates into practical solutions</p>
          </motion.div>
          
          {loading && (
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              {[...Array(3)].map((_, i) => (
                <Card key={i} className="neural-card"><CardContent className="p-6"><Skeleton className="h-64 w-full" /></CardContent></Card>
              ))}
            </div>
          )}
              ...
              {filteredCaseStudies.length > 0 ? (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  {filteredCaseStudies.map((study, index) => (
                    <motion.div key={study._id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: index * 0.2 }}>
                      <Card className="neural-card h-full overflow-hidden">
                        <div className="aspect-video bg-gradient-to-br from-primary/20 to-secondary/20 relative">
                        ...
                        <CardContent className="p-6">
                          ...
                          <div className="mb-4">
                            <div className="text-sm font-medium mb-2">Technologies:</div>
                            <div className="flex flex-wrap gap-2">
                              {study.technologies.map((tech) => ( <Badge key={tech} variant="outline" className="text-xs">{tech}</Badge>))}
                            </div>
                          </div>
                          <div className="mb-6">
                            <div className="text-sm font-medium mb-2">Key Results:</div>
                          ...
                          <Button variant="outline" className="btn-ghost-neural w-full">Read Full Case Study</Button>
                        </CardContent>
                      </Card>
                    </motion.div>
                  ))}
                 ...
                 <p className="text-center text-muted-foreground text-lg">
                  {caseStudies.length > 0 ? "No case studies found matching your search." : "No case studies available at the moment."}
                </p>
              )}
            </>
          )}
